...
def leer_alumno(mat):
    try:
        ...
        if alumno !=None:
            return jsonify({'alumno':alumno,'mensaje':'Alumno encontraso.','Exito':True})      
        else:
...
def registrar_alumno():
    try:
        ...
        return jsonify({'mensaje':'Alumno registrado exitosamente','exito':True})   
    except Exception as ex:
            return jsonify({'mensaje':'error de conexion {}'.format(ex)}) 
...
def modificar_alumno(mat):
    try:
        ...
        if alumno !=None:
            cursor=con.connection.cursor()
            sql="""UPDATE  alumnos SET nombre='{0}', 
            apaterno='{1}', 
            amaterno='{2}', 
            correo='{3}' 
            WHERE matricula={4}""".format(request.json['nombre'],
            request.json['apaterno'],
            request.json['amaterno'],
            request.json['correo'], mat)
            cursor.execute(sql)
            con.connection.commit()       
            return jsonify({'mensaje':'Alumno actualizado exitosamente','exito':True})
        else:
...
def eliminar_alumno(mat):
    try:
        ...
        if alumno !=None:
            ...
            cursor.execute(sql)
            con.connection.commit()       
            return jsonify({'mensaje':'Alumno eliminado exitosamente','exito':True})
